...
class IllustratorProjectPublishPlugin(HookBaseClass):
    """
    Plugin for publishing an illustrator project.
    ...
    def settings(self):
        ...
        base_settings = \
            super(IllustratorProjectPublishPlugin, self).settings or {}

        # settings specific to this class
        illustrator_publish_settings = {
            "Publish Template": {
                "type": "template",
                "default": None,
        ...
        base_settings.update(illustrator_publish_settings)
    ...
    def accept(self, settings, item):
        ...
        if not path:
            ...
            self.logger.warn(
                "The Illustrator project has not been saved.",
                extra=self.__get_save_as_action()
            )

        self.logger.info(
            "Illustrator '%s' plugin accepted." %
            (self.name,)
        )
        return {
            "accepted": True,
    ...
    def validate(self, settings, item):
        ...
        if not path:
            ...
            # validation fails.
            error_msg = "The Illustrator project '%s' has not been saved." % \
                        (item.name,)
            self.logger.error(
                error_msg,
                extra=self.__get_save_as_action()
        ...
        self.logger.debug("%s"%(work_template,))
        if work_template:
            if not work_template.validate(path):
                self.logger.warning(
                    ...
                    extra={
                        "action_button": {
                            "label": "Save File",
                            "tooltip": "Save the current Illustrator project"
        ...
        return super(IllustratorProjectPublishPlugin, self).validate(
            settings, item)
    ...
    def publish(self, settings, item):
        ...
        item.properties["path"] = path
        item.properties["publish_type"] = "Illustrator Project"

        # let the base class register the publish
        super(IllustratorProjectPublishPlugin, self).publish(settings, item)
    ...
    def finalize(self, settings, item):
        ...
        super(IllustratorProjectPublishPlugin, self).finalize(settings, item)
