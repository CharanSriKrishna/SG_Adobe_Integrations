...
class IllustratorSceneCollector(HookBaseClass):
    """
    Collector that operates on the current Illustrator document. Should inherit
    ...
    def settings(self):
        ...
        collector_settings = super(IllustratorSceneCollector, self).settings or {}

        # settings specific to this collector
        illustrator_session_settings = {
            "Work Template": {
                "type": "template",
                "default": None,
        ...
        collector_settings.update(illustrator_session_settings)

        return collector_settings

    def process_current_session(self, settings, parent_item):
        """
        Analyzes the open documents in Illustrator and creates publish items
    ...
    def __get_project_publish_item(self, settings, parent_item):
        ...
        if path:
            project_name = self.parent.engine.adobe.app.activeDocument.name
        ...
        self.logger.info(
            "Collected Illustrator document: {}".format(project_name))
        ...
        if path:
            project_item.set_thumbnail_from_path(os.path.dirname(path))
        ...
        if work_template is not None:
            ...
            self.logger.debug("Work template defined for Illustrator collection.")
        return project_item
