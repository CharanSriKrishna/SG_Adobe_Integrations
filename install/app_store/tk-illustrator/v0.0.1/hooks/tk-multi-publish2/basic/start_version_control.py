...
class IllustratorStartVersionControlPlugin(HookBaseClass):
    """
    Simple plugin to insert a version number into the Illustrator document path if
    one does not exist.
    """
    ...
    def accept(self, settings, item):
        ...
        if path:
            ...
            if version_number is not None:
                self.logger.info(
                    "Illustrator '%s' plugin rejected project" % self.name
                )
                self.logger.info(
            ...
            self.logger.warn(
                "Illustrator project has not been saved.",
                extra=self.__get_save_as_action()
            )

        self.logger.info(
            "Illustrator '%s' plugin accepted the project." % self.name,
    ...
    def validate(self, settings, item):
        ...
        if not path:
            ...
            # validation fails
            error_msg = "The Illustrator project has not been saved."
            self.logger.error(
                error_msg,
                extra=self.__get_save_as_action()
            )
    ...
    def publish(self, settings, item):
        ...
        self.logger.info(
            "A version number has been added to the Illustrator project...")
        self.logger.info("  Illustrator project path: %s" % (version_path,))
