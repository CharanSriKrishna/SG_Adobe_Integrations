...
class IllustratorLauncher(SoftwareLauncher):
    """
    Handles the launching of Illustrator. Contains the logic for
    scanning for installed versions of the software and
    how to correctly set up a launch environment for the tk-illustrator
    engine.
    """
    ...
    EXECUTABLE_MATCH_TEMPLATES = {
        ...
        "win32": "C:/Program Files/Adobe/Adobe Illustrator {version}/Support Files/Contents/Windows/Illustrator.exe",
    }
    ...
    def prepare_launch(self, exec_path, args, file_to_open=None):
        """
        Prepares an environment to launch illustrator so that will automatically
        ...
        args = os.path.join(self.disk_location, "resources", "Untitled.ai")

        return LaunchInformation(exec_path, args, required_env)

    def scan_software(self):
        """
        Scan the filesystem for all illustrator executables.

        :return: A list of :class:`SoftwareVersion` objects.
        """

        self.logger.debug("Scanning for illustrator executables...")
        ...
        if sys.platform not in self.EXECUTABLE_MATCH_TEMPLATES:
            self.logger.debug("illustrator not supported on this platform.")
            return []
    ...
    def compute_environment(self):
        """
        Return the env vars needed to launch the illustrator plugin.

        This will generate a dictionary of environment variables
        needed in order to launch the illustrator plugin.
        ...
        env["SHOTGUN_ENGINE"] = "tk-illustrator"
    ...
    def __get_adobe_framework_location(self):
        ...
        engine_desc = env.get_engine_descriptor("tk-illustrator")
        if env_name is None:
            self.logger.warn(
                ("The current environment {!r} "
                 "is not configured to run the tk-illustrator "
        ...
        for req_framework in engine_desc.get_required_frameworks():
            ...
            self.logger.warn(
                ("The engine tk-illustrator must have "
