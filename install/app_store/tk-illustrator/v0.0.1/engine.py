...
class IllustratorEngine(sgtk.platform.Engine):
    ...
    _DIALOG_PARENT = None
    _WIN32_ILLUSTRATOR_MAIN_HWND = None
    _PROXY_WIN_HWND = None
    _HEARTBEAT_DISABLED = False
    _PROJECT_CONTEXT = None
    _ILLUSTRATOR_PID = None
    _POPUP_CACHE = None
    _ILLUSTRATOR_WIN32_DIALOG_WINDOW_CLASS = "#32770" # the windows window class name used by Illustrator for modal dialogs
    __WIN32_GW_CHILD = 5
    _CONTEXT_CACHE_KEY = "illustrator_context_cache"
    ...
    def post_context_change(self, old_context, new_context):
        ...
        # propagated down to any of Illustrator's subprocesses.
    ...
    def pre_app_init(self):
        ...
        self.__tk_illustrator = self.import_module("tk_illustrator")
        self.logger.debug(self.__tk_illustrator)
        ...
        self._adobe = self.__tk_illustrator.AdobeBridge.get_or_create(
        ...
        shotgun_data = self.__tk_illustrator.shotgun_data
        settings = self.__tk_illustrator.shotgun_settings
        # keep a handle for shotgun globals as they are needed in other
        # functions as well
        self.__shotgun_globals = self.__tk_illustrator.shotgun_globals
    ...
    def destroy_engine(self):
        ...
        # instead of Illustrator's application window.
        if self._PROXY_WIN_HWND and sys.platform == "win32":
            self.__tk_illustrator.win_32_api.SetParent(self._PROXY_WIN_HWND, 0)
    ...
    def register_command(self, name, callback, properties=None):
        ...
        return super(IllustratorEngine, self).register_command(
            name,
            callback,
            properties,
        )
    ...
    def host_info(self):
        ...
        :returns: A {"name": application name, "version": application version}
                  dictionary. eg: {"name": "Illustrator", "version": "2017.1.1"}
        """
        if not self.adobe:
        ...
        # Adobe Illustrator Version: 2017.1.1 20170425.r.252 2017/04/25:23:00:00 CL 1113967  x64\rNumber of .....
        # and use it instead if available.
        #regex = re.compile("WIN ([0-9]+)(, [0-9]+)*")
    ...
    def save(self, path=None):
        ...
        with self.context_changes_disabled():
            ...
            if path is None:
                self.adobe.app.activeDocument.save()
            else:
                ensure_folder_exists(os.path.dirname(path))
                self.adobe.app.activeDocument.saveAs(self.adobe.File(path))
    ...
    def save_as(self):
        ...
        file_dialog = QtGui.QFileDialog(
            ...
            caption="Save As",
            directory=doc_path,
            filter="Illustrator Documents (*.ai)"
        )
    ...
    def _check_connection(self):
        ...
        # that the context is correct. Since we rely of Illustrator sending
    ...
    def _handle_active_document_change(self, active_document_path):
        ...
        # which is useful when an app is doing a lot of Illustrator work that
        ...
        with self.heartbeat_disabled():
            ...
            if cached_context:
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Unable to determine context from path. Setting the Project context.{e}"
                    )
    ...
    def _run_tests(self):
        """
        Runs the test suite for the tk-illustrator bundle.
        """
    ...
    def app_id(self):
        ...
        PHSP for Illustrator, or AEFT for After Effect.
        """
    ...
    def _define_qt_base(self):
        ...
        * dialog_base - base class for to use for Toolkit's dialog factory
        ...
        base = super(IllustratorEngine, self)._define_qt_base()
        if not base:
    ...
    def _override_qmessagebox(self, q_message_box):
        ...
        separate process, they will pop up behind the Illustrator window. Wrap
    ...
    def __check_for_popups(self):
        """
        Method will check if a popup dialog from Illustrator was openend
        ...
        if sys.platform == "win32":
            ...
            # to get all dialogs from Illustrator, we have to query the
            # Illustrator process id first. As this code runs inside a
            # separate python process, we use a subprocess for this.
            if self._ILLUSTRATOR_PID == -1:
                return
            elif self._ILLUSTRATOR_PID is None:
                # the windows tasklist command will provide the process id
                # of the Illustrator executable. As there is always only one
                # instance of Illustrator, this should be safe to determine the
                # process id.
                ...
                # where 1234 describes the pid of Illustrator
                match = re.match("[^0-9]+([0-9]+).*", six.ensure_str(out_string), re.DOTALL)
                if not match:
                    self._ILLUSTRATOR_PID = -1
                self._ILLUSTRATOR_PID = int(match.group(1))

            # with the process id of Illustrator, we can get all HWNDS that point to
            # dialog classes.
            hwnds = self.__tk_illustrator.win_32_api.find_windows(
                                process_id=self._ILLUSTRATOR_PID,
                                class_name=self._ILLUSTRATOR_WIN32_DIALOG_WINDOW_CLASS,
                                stop_if_found=True,
                            )
            ...
            all_hwnds = {}
            GetWindow = self.__tk_illustrator.win_32_api.ctypes.windll.user32.GetWindow
            for hwnd in hwnds:
            ...
            for hwnd_long in new_hwnds:
                self.__tk_illustrator.win_32_api.bring_to_front(all_hwnds[hwnd_long])
    ...
    def _win32_get_illustrator_main_hwnd(self):
        """
        Windows specific method to find the main Illustrator window
        handle (HWND)
        """
        if not self._WIN32_ILLUSTRATOR_MAIN_HWND:
            for major in sorted(self.__CC_VERSION_MAPPING.keys()):
                for minor in range(10):
                    found_hwnds = self.__tk_illustrator.win_32_api.find_windows(
                        class_name="Adobe Illustrator".format(major, minor),
                        stop_if_found=True,
                    )

                    if found_hwnds:
                        self._WIN32_ILLUSTRATOR_MAIN_HWND = found_hwnds[0]
                        return self._WIN32_ILLUSTRATOR_MAIN_HWND
    ...
    def _win32_get_proxy_window(self):
        ...
        Toolkit dialogs.  This will be parented to the main Illustrator
        application.

        :returns: A QWidget that has been parented to Illustrator's window.
        """
        # Get the main Illustrator window:
        ps_hwnd = self._win32_get_illustrator_main_hwnd()
        ...
        if ps_hwnd:
            ...
            if QtCore.__version__.startswith("4."):
                proxy_win_hwnd = self.__tk_illustrator.win_32_api.qwidget_winid_to_hwnd(
                ...
                # to find Illustrator's main window. To do that, we actually have
                ...
                try:
                    proxy_win_hwnd_found = self.__tk_illustrator.win_32_api.find_windows(
                        stop_if_found=True,
            ...
            self.logger.debug(
                "Unable to determine the HWND of Illustrator itself. This means "
                "that we can't properly setup window parenting for Toolkit apps."
            )

        # Parent to the Illustrator application window if we found everything
        ...
        if proxy_win_hwnd is None:
            self.logger.warning(
                ...
                "not be parented to Illustrator, but they will still function "
                "properly otherwise."
            )
        else:
            # Set the window style/flags. We don't need or want our Python
            # dialogs to notify the Illustrator application window when they're
            # opened or closed, so we'll disable that behavior.
            win_ex_style = self.__tk_illustrator.win_32_api.GetWindowLong(
                proxy_win_hwnd,
                self.__tk_illustrator.win_32_api.GWL_EXSTYLE,
            )

            self.__tk_illustrator.win_32_api.SetWindowLong(
                proxy_win_hwnd,
                self.__tk_illustrator.win_32_api.GWL_EXSTYLE,
                win_ex_style | self.__tk_illustrator.win_32_api.WS_EX_NOPARENTNOTIFY,
            )
            self.__tk_illustrator.win_32_api.SetParent(proxy_win_hwnd, ps_hwnd)
    ...
    def show_dialog(self, title, bundle, widget_class, *args, **kwargs):
        ...
        # can cause a crash in Illustrator as the system may try to send
        ...
        # appear behind the main Illustrator window
