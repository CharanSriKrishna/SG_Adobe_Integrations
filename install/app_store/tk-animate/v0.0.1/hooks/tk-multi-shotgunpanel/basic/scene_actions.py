...
class AnimateActions(HookBaseClass):
    ...
    def execute_action(self, name, params, sg_publish_data):
        ...
        if name == _ADD_TO_PROJECT:
            self._import_proj(path, is_sequence)
        
        if name == _OPEN_FILE:
            self.open_file(path, is_sequence)

        if name == _ADD_AS_A_LAYER:
            self.add_as_layer(path, is_sequence)


    ###########################################################################
    # helper methods

    def _import_proj(self, path, is_sequence):
        """
        Helper method to add the project to the current project
        """
        import_file_path = self.parent.engine._flashify_path(path)
        #self.parent.engine.adobe.rpc_eval("fl.openDocument('%s')" % import_file_path)
        self.parent.engine.adobe.rpc_eval("fl.getDocumentDOM().importFile('%s')"% import_file_path)
    
    def add_as_layer(self, path):
        pass
