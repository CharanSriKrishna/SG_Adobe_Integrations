...
class AnimateStartVersionControlPlugin(HookBaseClass):
    """
    Simple plugin to insert a version number into the Animate document path if
    one does not exist.
    """
    ...
    def accept(self, settings, item):
        ...
        if path:
            ...
            if version_number is not None:
                self.logger.info(
                    "Animate '%s' plugin rejected project" % self.name
                )
                self.logger.info(
            ...
            self.logger.warn(
                "Animate project has not been saved.",
                extra=self.__get_save_as_action()
            )

        self.logger.info(
            "Animate '%s' plugin accepted the project." % self.name,
    ...
    def validate(self, settings, item):
        ...
        if not path:
            ...
            # validation fails
            error_msg = "The Animate project has not been saved."
            self.logger.error(
                error_msg,
                extra=self.__get_save_as_action()
            )
    ...
    def publish(self, settings, item):
        ...
        self.logger.info(
            "A version number has been added to the Animate project...")
        self.logger.info("  Animate project path: %s" % (version_path,))
