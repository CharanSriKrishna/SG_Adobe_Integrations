...
class AnimateProjectPublishPlugin(HookBaseClass):
    """
    Plugin for publishing an animate project.
    ...
    def settings(self):
        ...
        base_settings = \
            super(AnimateProjectPublishPlugin, self).settings or {}

        # settings specific to this class
        animate_publish_settings = {
            "Publish Template": {
                "type": "template",
                "default": None,
        ...
        base_settings.update(animate_publish_settings)
    ...
    def accept(self, settings, item):
        ...
        if not path:
            ...
            self.logger.warn(
                "The Animate project has not been saved.",
                extra=self.__get_save_as_action()
            )

        self.logger.info(
            "Animate '%s' plugin accepted." %
            (self.name,)
        )
        return {
            "accepted": True,
    ...
    def validate(self, settings, item):
        ...
        if not path:
            ...
            # validation fails.
            error_msg = "The Animate project '%s' has not been saved." % \
                        (item.name,)
            self.logger.error(
                error_msg,
                extra=self.__get_save_as_action()
            ...
            if not work_template.validate(path):
                self.logger.warning(
                    ...
                    extra={
                        "action_button": {
                            "label": "Save File",
                            "tooltip": "Save the current Animate project"
        ...
        return super(AnimateProjectPublishPlugin, self).validate(
            settings, item)
    ...
    def publish(self, settings, item):
        ...
        item.properties["path"] = path
        item.properties["publish_type"] = "Animate Project"

        # let the base class register the publish
        super(AnimateProjectPublishPlugin, self).publish(settings, item)
    ...
    def finalize(self, settings, item):
        ...
        super(AnimateProjectPublishPlugin, self).finalize(settings, item)
