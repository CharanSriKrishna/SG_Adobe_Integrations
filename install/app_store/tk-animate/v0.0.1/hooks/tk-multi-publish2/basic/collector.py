...
class AnimateSceneCollector(HookBaseClass):
    """
    Collector that operates on the current Animate document. Should inherit
    ...
    def settings(self):
        ...
        collector_settings = super(AnimateSceneCollector, self).settings or {}

        # settings specific to this collector
        animate_session_settings = {
            "Work Template": {
                "type": "template",
                "default": None,
        ...
        collector_settings.update(animate_session_settings)

        return collector_settings

    def process_current_session(self, settings, parent_item):
        """
        Analyzes the open documents in Animate and creates publish items
    ...
    def __get_project_publish_item(self, settings, parent_item):
        ...
        if path:
            project_name = self.parent.engine.adobe.rpc_eval("fl.getDocumentDOM().name")
        ...
        self.logger.info(
            "Collected Animate document: {}".format(project_name))
        ...
        if work_template is not None:
            ...
            self.logger.debug("Work template defined for Animate collection.")
        return project_item
