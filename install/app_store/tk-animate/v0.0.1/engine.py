...
class AnimateEngine(sgtk.platform.Engine):
    MAX_THUMB_SIZE = 512
    ...
    _DIALOG_PARENT = None
    _WIN32_ANIMATE_MAIN_HWND = None
    _PROXY_WIN_HWND = None
    _HEARTBEAT_DISABLED = False
    _PROJECT_CONTEXT = None
    _ANIMATE_PID = None
    _POPUP_CACHE = None
    _ANIMATE_WIN32_DIALOG_WINDOW_CLASS = "#32770" # the windows window class name used by Animate for modal dialogs
    __WIN32_GW_CHILD = 5
    _CONTEXT_CACHE_KEY = "animate_context_cache"
    ...
    def pre_app_init(self):
        ...
        self.__tk_animate = self.import_module("tk_animate")
        self.logger.debug(self.__tk_animate)
        ...
        self._adobe = self.__tk_animate.AdobeBridge.get_or_create(
        ...
        shotgun_data = self.__tk_animate.shotgun_data
        settings = self.__tk_animate.shotgun_settings
        # keep a handle for shotgun globals as they are needed in other
        # functions as well
        self.__shotgun_globals = self.__tk_animate.shotgun_globals
    ...
    def destroy_engine(self):
        ...
        if self._PROXY_WIN_HWND and sys.platform == "win32":
            self.__tk_animate.win_32_api.SetParent(self._PROXY_WIN_HWND, 0)
    ...
    def register_command(self, name, callback, properties=None):
        ...
        return super(AnimateEngine, self).register_command(
            name,
            callback,
            properties,
        )
    ...
    def host_info(self):
        ...
        :returns: A {"name": application name, "version": application version}
                  dictionary. eg: {"name": "Animate", "version": "2017.1.1"}
        """
        if not self.adobe:
            # Don't error out if the bridge was not yet started
            return {"name": "Animate", "version": "unknown"}

        version = self.adobe.app.version
        # app.animate.AnimateVersion just returns 18.1.1 which is not what users see in the UI
        # extract a more meaningful version from the systemInformation property
        # which gives something like:
        # Adobe Animate Version: 2017.1.1 20170425.r.252 2017/04/25:23:00:00 CL 1113967  x64\rNumber of .....
    ...
    def project_path(self):
        ...
        :rtype: str
        """
        with self.adobe.response_logging_silenced():
            try:
                path = self.adobe.rpc_eval("fl.getDocumentDOM() ? fl.getDocumentDOM().path : undefined")
            except RuntimeError:
                path = None
        if path is not None:
        ...
        if path == "undefined":
            path = None
        return self.sanitise_path(path)
    ...
    def save(self, path=None):
        ...
        with self.context_changes_disabled():
            ...
            if path is None:
                self.adobe.rpc_eval("fl.saveDocument(fl.getDocumentDOM())")

            else:
                ensure_folder_exists(os.path.dirname(path))
                self.adobe.rpc_eval("fl.saveDocument(fl.getDocumentDOM(), '%s')" % self._flashify_path(path))
    ...
    def _handle_active_document_change(self, active_document_path):
        ...
        if active_document_path is not None:
            active_document_path = self.sanitise_path(active_document_path)
    ...
    def _define_qt_base(self):
        ...
        * dialog_base - base class for to use for Toolkit's dialog factory
        ...
        base = super(AnimateEngine, self)._define_qt_base()
        if not base:
    ...
    def __check_for_popups(self):
        """
        Method will check if a popup dialog from Animate was openend
        ...
        if sys.platform == "win32":
            ...
            # to get all dialogs from Animate, we have to query the
            # Animate process id first. As this code runs inside a
            # separate python process, we use a subprocess for this.
            if self._ANIMATE_PID == -1:
                return
            elif self._ANIMATE_PID is None:
                # the windows tasklist command will provide the process id
                # of the Animate executable. As there is always only one
                # instance of Animate, this should be safe to determine the
                # process id.
                pid_query_process = subprocess.Popen(
                    ['tasklist', '/FI', 'ImageName eq Animate.exe', '/FO', 'CSV', '/NH'],
                ...
                # where 1234 describes the pid of Animate
                match = re.match("[^0-9]+([0-9]+).*", six.ensure_str(out_string), re.DOTALL)
                if not match:
                    self._ANIMATE_PID = -1
                self._ANIMATE_PID = int(match.group(1))

            # with the process id of Animate, we can get all HWNDS that point to
            # dialog classes.
            hwnds = self.__tk_animate.win_32_api.find_windows(
                                process_id=self._ANIMATE_PID,
                                class_name=self._ANIMATE_WIN32_DIALOG_WINDOW_CLASS,
                                stop_if_found=True,
                            )
            ...
            all_hwnds = {}
            GetWindow = self.__tk_animate.win_32_api.ctypes.windll.user32.GetWindow
            for hwnd in hwnds:
            ...
            for hwnd_long in new_hwnds:
                self.__tk_animate.win_32_api.bring_to_front(all_hwnds[hwnd_long])
    ...
    def _win32_get_animate_main_hwnd(self):
        ...
        handle (HWND)
        """
        if not self._WIN32_ANIMATE_MAIN_HWND:
            for major in sorted(self.__CC_VERSION_MAPPING.keys()):
                for minor in range(10):
                    found_hwnds = self.__tk_animate.win_32_api.find_windows(
                        class_name="ANIMATE_CApplication_{}.{}".format(major, minor),
                        stop_if_found=True,
                    )

                    if found_hwnds:
                        self._WIN32_ANIMATE_MAIN_HWND = found_hwnds[0]
                        return self._WIN32_ANIMATE_MAIN_HWND
    ...
    def _win32_get_proxy_window(self):
        ...
        if ps_hwnd:
            ...
            if QtCore.__version__.startswith("4."):
                proxy_win_hwnd = self.__tk_animate.win_32_api.qwidget_winid_to_hwnd(
                ...
                # to find Animate's main window. To do that, we actually have
                ...
                try:
                    proxy_win_hwnd_found = self.__tk_animate.win_32_api.find_windows(
                        stop_if_found=True,
            ...
            self.logger.debug(
                "Unable to determine the HWND of Animate itself. This means "
                "that we can't properly setup window parenting for Toolkit apps."
            )

        # Parent to the Animate application window if we found everything
        ...
        if proxy_win_hwnd is None:
            self.logger.warning(
                ...
                "not be parented to Animate, but they will still function "
                "properly otherwise."
            )
        else:
            ...
            win_ex_style = self.__tk_animate.win_32_api.GetWindowLong(
                proxy_win_hwnd,
                self.__tk_animate.win_32_api.GWL_EXSTYLE,
            )

            self.__tk_animate.win_32_api.SetWindowLong(
                proxy_win_hwnd,
                self.__tk_animate.win_32_api.GWL_EXSTYLE,
                win_ex_style | self.__tk_animate.win_32_api.WS_EX_NOPARENTNOTIFY,
            )
            self.__tk_animate.win_32_api.SetParent(proxy_win_hwnd, ps_hwnd)
    ...
    def _flashify_path(self, path):
        path = str.replace(path, "\\", "/")
        path = str.replace(path, ":", "|")
        path = str.replace(path, " ", "%20")
