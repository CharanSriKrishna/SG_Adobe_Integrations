...
sg_manager.Manager = new function() {
    ...
    const _app_is_supported = function() {
        ...
        // added this for integration support
        const app_name = _cs_interface.hostEnvironment.appName 
        if (app_name == 'FLPR'){
            return true
        }
    ...
    const _active_document_check = function(event) {
        ...
        const val = None 
        if (app_name == 'FLPR'){
            val =  "(fl.getDocumentDOM() ? fl.getDocumentDOM().pathURI : undefined)"
        }
        if (app_name == 'PPRO'){
            val =  "app.project.path"
        }
        if (app_name == 'ILST'){
            val =  "app.activeDocument.fullName"
        }
        else{
            val = "app.activeDocument.fullName.fsName"
        }
        // added this for integration support
    ...
    const _on_server_port_found = function(port) {
        ...
        const app_name = _cs_interface.hostEnvironment.appName
        if (app_name == "FLPR"){
            const result = process.env.SHOTGUN_ADOBE_FRAMEWORK_LOCATION
    ...
    const _setup_event_listeners = function() {
        ...
        // ---- Events from the panel_setup_event_listeners
