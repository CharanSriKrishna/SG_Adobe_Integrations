var basePath = fl.scriptURI.replace(/^(file:\/{3}.+\/js\/)dhx\/.*$/i, "$1");
fl.runScript(basePath + "shotgun/ECMA/JSON-js/json2.js");
...
function wrap_item(item, name) {
    if (item instanceof Array) {
        var wrappers = [];
        for (i in item) {
            var r = item[i].reflect;
            var itemName = r!=undefined ? r.name : i;

            wrappers.push(wrap_item(item[i], itemName));
        }
